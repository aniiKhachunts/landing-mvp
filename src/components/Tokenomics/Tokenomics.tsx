                ...
                <div className="tokenomics__grid">
                    <div className="tokenomics__chart" role="img" aria-label="Token allocation pie chart">
                        <ResponsiveContainer width="100%" height={260}>
                            <PieChart>
                                <Pie data={data} dataKey="value" nameKey="name" innerRadius={70} outerRadius={110} paddingAngle={1.5}>
                                    {data.map((entry, i) => (
                                        <Cell key={`slice-${entry.name}`} fill={colors[i % colors.length]} />
                                    ))}
                                </Pie>
                                <Tooltip formatter={(v: number, n: string) => [`${v}%`, n]} contentStyle={{ background: '#0b0f14', border: '1px solid rgba(255,255,255,.08)' }} />
                            </PieChart>
                        </ResponsiveContainer>
                    </div>
